...
import 'main.dart'; // To route to MainShell
...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  bool _enableGps = false; // NEW: GPS State

  Future<void> _saveProfile() async {
    if (_nameController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Name is required", style: TextStyle(color: Colors.white)), backgroundColor: Colors.red));
      return;
    }
    ...
    await prefs.setBool('enable_gps', _enableGps); // Save GPS preference
  ...
  // Request permission at the moment of toggle
  Future<void> _toggleGps(bool value) async {
    if (value) {
      ...
      if (permission == LocationPermission.denied || permission == LocationPermission.deniedForever) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("GPS Permission Denied. Cannot enable location.")));
        return;
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          const Text("Welcome! Please set up your profile.", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
